...
with depthai.Device(pipeline) as device:
    ...
    map_size = 200
    robot_map = np.zeros((map_size, map_size), dtype=np.uint8)
    ...
    while True:
        ...
        # Actualizar el mapa
        # Mapear el desplazamiento a las coordenadas del mapa
        map_x = int(map_size / 2 + displacement_pixels)
        map_y = int(map_size / 2)
        
        # Limitar las coordenadas al tamaño del mapa
        map_x = max(0, min(map_size - 1, map_x))
        map_y = max(0, min(map_size - 1, map_y))

        # Incrementar el valor en el mapa
        robot_map[map_y, map_x] += 1

        # Mostrar el mapa
        cv2.imshow("Robot Map", cv2.resize(robot_map, (400, 400)))
